...
<body id="install">
<?php
$form = OForm::factory('install/shell')
	->title($title)
	->content('install_content', $content);

if (!isset($complete)) {
	$form->button('next', OField::factory('submit')->name('next')->default_value('Next'));
}

echo $form->attributes(array(
		'class' => 'edit',
	))
	->attributes(array(
		'class' => 'box box-install frm',
	), true);
?>
</body>
</html>
