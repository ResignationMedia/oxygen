	...
	Popover.prototype = {
		...
		bindEvents: function () {
			...
			$('body').click($.proxy(function (e) {
				if (this.popoverIsOpen()) {
					var $target = $(e.target);
					if (!$(this.$popover).has($target).size() && !$(this.$popover).filter($target).size() && !$target.parents().hasClass('ui-widget')) {
						this.hide();
					}
				}
		...
		showPopover: function (e) {
			...
			// TODO keep or move? (160-182)

			// Close button?
			this.$popover.find('.cf-popover-close').click($.proxy(function(e){
				if (this.popoverIsOpen()) {
					this.hidePopover(e);
				}
			}, this));

			var href = this.$trigger.data('href');
			if (typeof href !== 'undefined' && !this.$popover.hasClass('popoverajax-content-loaded')) {
				// Set loading animation
				this.$popover.find('.before').after('<div class="popoverajax-content-loading">Loading...</div>');

				var $this = this;
				$.getJSON(href, function(response){
					if (response.result == 'success') {
						$this.$popover.find('.popoverajax-content-loading').remove().end().find('.after').before(response.html);
						$this.$popover.addClass('popoverajax-content-loaded');
						$this.$trigger.trigger('popoverajax-content-loaded');
					}
					else {
						// TODO error handling
					}
		...
		hidePopover: function (e) {
			...
			e.stopPropagation();
			this.hide();
			this.$trigger.trigger('popover-closed');
		},
